...
class APK:
    def __init__(
        ...
        self._fore_service_count, self._a_service_count = self._has_suspicious_service()
        ...
        self._suspicious_libs = len(self._suspicious_libraries())
    ...
    def get_metadata(self):
        ...
        return {
            ...
            "accessibility_service_count": self._a_service_count,
            "foreground_service_count": self._fore_service_count,
    ...
    def _suspicious_libraries(self):
        ...
        all_so_files = []
        for file_name in self._apk.get_files():
        ...
        for so_path in all_so_files:
            ...
            # Convert filename to expected declared name: 'lib evil.so' -> 'evil'
            # Library should start with 'lib' and end with '.so' to be considered
            if so_filename.startswith("lib") and so_filename.endswith(".so"):
                expected_declared_name = so_filename[3:-3]  # Remove 'lib' and '.so'

                # Handle version suffixes: 'foo-1.2' -> 'foo'
                ...
                if expected_declared_name not in declared_libs:
                    suspicious.append(expected_declared_name)
        return suspicious
    ...
    def _has_suspicious_service(self) -> list[int]:
        ...
        for service in self._services:
            ...
            for kw in suspicious_keywords:
                if kw in service_lower:
                    if "accessibility" in service_lower:
                        accessibility_count += 1
                    if "foreground" in service_lower:
                        foreground_count += 1
        return [foreground_count, accessibility_count]
        ...
        with open(output_csv, mode=mode, newline="", encoding="utf-8") as csv_file:
            ...
            for metadata in self.results:
                ...
                row = {
                    ...
                    "accessibility_service_count": metadata.get(
                        "accessibility_service_count", 0
                    ),
                    "foreground_service_count": metadata.get(
    ...
    def output_features(self):
        ...
        for metadata in self.results:
            if metadata.get("error"):
                ...
                print(
                    f"Accessibility Service Count: {metadata['accessibility_service_count']}"
                )
                print(
                    f"Foreground Service Count: {metadata['foreground_service_count']}"
                )
                print(
